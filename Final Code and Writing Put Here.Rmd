...
Initially, pairs of every variable were graphed to get an idea of which variables looked interesting and how they work with each other. Also, a correlation matrix was useful in determining which quantitative variables were correlated with each other, as seen below. The Speed and Height variables were found to be highly correlated at .95. To account for this correlation, only the Speed variable was used when building the model.
...
```{r}
round(vcov(coaster.model),2) #prints a lot
```
