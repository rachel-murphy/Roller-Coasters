...
k<-kmeans(coaster_data_cat, centers=4, iter.max=200, nstart=50)
...
```{r, warning=FALSE, echo=FALSE}
n <- nrow(scale(coaster_data_cat))
wss <- rep(0, 20)
wss[1] <- (n - 1) * sum(apply(scale(coaster_data_cat), 2,var))
for (i in 2:20){
  wss[i] <- sum(kmeans(scale(coaster_data_cat[,1:6]), centers = i, iter.max=100, nstart=25)$withinss)
...
```{r, warning=FALSE, echo=FALSE}
 clusplot(coaster_data_cat, k$cluster, color=TRUE, shade=TRUE, 
     labels=5, lines=0, col.p = "black", main = "Three Clusters of Kmeans")
```

```{r, warning=FALSE, echo=FALSE}
pairs(coaster_data_cat)

lm(cbind(score, Score.2) ~  Factor.launch+ Factor.restraint + Factor.seating + Factor.material + length.x + num_inversions + speed.x, data = coaster_data_cat)
```
